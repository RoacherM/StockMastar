...
**类型约束（违反则前端渲染崩溃，务必遵守）：**
- 所有价格字段（top, bottom, price）必须是 **JSON number**，如 `15.5`，绝不能是字符串 `"15.5"`
- 所有日期字段必须是 **字符串**，格式 `"YYYY-MM-DD"`
- zones.type 只能是 `"accumulation"` 或 `"distribution"`，不能用其他值
- events.type 只能是以下代码之一：PS, SC, BC, AR, ST, Spring, UTAD, SOS, SOW, LPS, LPSY, JAC, BU
...
```json
{
  ...
  "quote": "一句话市场评语",
  "phases": [
    { "name": "Phase A", "start": "2024-01-18", "end": "2024-04-15", "description": "阶段描述" }
  ],
  "zones": [
